var DiffCamEngine = (function() {
	...
	function init(options) {
		...
		captureWidth = options.captureWidth || 320;
		captureHeight = options.captureHeight || 180;
		diffWidth = options.diffWidth || 32;
		diffHeight = options.diffHeight || 18;
		...
		if (options.stream) {
			stream = options.stream;
                        var that = this;
			setTimeout(function() { initSuccessCallback.apply(that); },1);
		} else {
			requestWebcam();
		}
	}
